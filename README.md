...
- You wil have to do these steps for EVERY new project you wish to push to github
1. Open the terminal and make sure it is in the correct directory
2. Initialize repo, tell gitbash to track. Terminal Command: `git init` <- git initialize

3. Create the online repo to connect with. Go to github.com --> click `new repo`
4. Give it a name and hit `Create Repository`
5. Copy `git remote add oigin` command and paste in vsc terminal
...
1. `git add .` - add all files in this directory to a new version. 
  - ** This does not commit, it just stages changes, these changes can still be over written**
2. `git add -m "commit message" - this command
